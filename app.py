...
def display_filterable_table(df_in, key_id):
    ...
    gb.configure_default_column(resizable=True, filterable=True, sortable=True, width=80, minWidth=40)
    ...
    if "台ゾロ目タイプ" in df_filtered.columns:
        gb.configure_column("台ゾロ目タイプ", width=80)

    # 5. 設置状態 -> 幅60px
    if "設置" in df_filtered.columns: 
        gb.configure_column("設置", width=80, cellStyle=style_status)
